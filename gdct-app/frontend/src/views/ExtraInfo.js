...
function getStyles(sysRole, sysRoles, theme) {
  return {
    fontWeight: sysRoles.indexOf(sysRole) === -1
...
export default function SignUp({ parentHandleChange }) {
  ...
  const { appSysRoles } = useSelector(
    (state) => ({
      appSysRoles: selectFactoryRESTResponseTableValues(
        selectAppSysRolesStore,
      )(
        state,
      ),
    }),
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <Select
                ...
                MenuProps={MenuProps}
              >
                {appSysRoles.length !== 0 && appSysRoles.map((sysRole) => (
                  <MenuItem
                    key={sysRole._id}
