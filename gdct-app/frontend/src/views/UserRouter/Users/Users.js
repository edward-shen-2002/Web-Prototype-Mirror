...
const UsersTable = () => {
  ...
  const handleClear = () => {
    setUserName(''); setLastName(''); setFirstName(''); setOrgId(''); setOrgName('');
  }
  ...
  const handleClick = () => {
    ...
    dispatch(getUsersRequest({params: {"username": u, "lastName": l, "firstName": f, "sysRole.org.orgId": o, "sysRole.org.orgName": n}}))
  }
  
  const { users } = useSelector((state) => ({
      users: selectFactoryRESTResponseTableValues(selectUsersStore)(
        state
      ),
    }))
  ...
  const localization = useMemo(
    () => ({ toolbar: {searchPlaceholder: 'Filter returned data', searchTooltip: 'Further filter the returned dataset with this filter bar'} }),
    []
  )
