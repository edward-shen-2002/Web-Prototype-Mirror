...
const AppRoleResourceController = (() => {
  const AppRoleResourceAxios = axios.create({
    baseURL: 'http://localhost:3000/role_manager/approleresources',
    withCredentials: true,
  })
  return {
    fetchAppRoleResource: async (_id) =>
      AppRoleResourceAxios.get(`/${_id}`).then(
        (res) => res.data.AppRoleResource
      ),
    fetch: async (_) =>
      AppRoleResourceAxios.get('').then((res) => res.data.AppRoleResources),
    create: async (AppRoleResource) =>
      AppRoleResourceAxios.post('', { AppRoleResource }).then(
        (res) => res.data.AppRoleResource
      ),
