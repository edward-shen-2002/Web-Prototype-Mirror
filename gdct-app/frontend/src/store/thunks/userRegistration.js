...
const getOrgGroup = () => {
  return(organizationGroupController.fetch()
...
const getOrg = (orgGroup) => {
  ...
  return(organizationController.fetchByOrgGroupId(orgGroup)
      .then((organizations) => {
        let options = []
...
const getProgram = (programInfo) => {
  ...
  return(programController.fetchByIds(programId)
      .then((programs) => {
        let options = [];
...
const getTemplateType = (userPrograms) => {
  ...
  return(templateTypeController.fetchByProgramIds(programList)
      .then((templateTypes) => {
        let submissionList = [];
        let index = 0;
...
const sendRegistrationData = (registerData) => {
  return(userController.create(registerData)
      .catch((error) => console.error(error))
  )
}
...
export const changeSubmission = () => (dispatch, getState) => {
  const {userRegistrationStore: {userSubmissions},} = getState()
...
export const searchOrganization = () => (dispatch, getState) => {
  const {userRegistrationStore: {searchKey, reference, organizationOptions},} = getState()
...
export const submit = () => (dispatch, getState) => {
  const {userRegistrationStore: {userSubmissions, registrationData, userAppSys},} = getState()
