...
import { selectWorkflowLinks, selectWorkflowNodes, selectWorkflowName, selectWorkflowId } from '../WorkflowStore/selectors'
...
export const updateWorkflow = () => (dispatch, getState) => {
  workflowController.update(_createWorkflow(dispatch, getState))
...
export const submitWorkflow = () => (dispatch, getState) => {
  workflowController.create(_createWorkflow(dispatch, getState))
    .catch((error) => dispatch(WorkflowStoreActions.UPDATE_WORKFLOW_ERROR(error)))
}