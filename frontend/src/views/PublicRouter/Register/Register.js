...
const defaultRoleControlConfig = {
  organizationOptions: [],
  userOrganization: [],
  programOptions: [],
  userProgram: [],
...
const ButtonBox = ({ activeStep, handleBack, handleNext, ableToComplete }) => (
  ...
  >
    ...
    <Button
      disabled={!ableToComplete || activeStep !== 2}
      variant="outlined"
      color="primary"
      className="register__button"
      onClick={handleNext}
        ...
        <>
          ...
          >
            <FormControl component="fieldset" className="register__selection">
              <FormLabel className="register__title" component="legend">
                Step1: Select your Organization Group
              </FormLabel>
              <RadioGroup
                aria-label="OrgGroup"
                name="OrgGroup1"
                defaultValue={organizationGroup}
                onChange={handleOrgGroupChange}
              >
                <FormControlLabel
                  value="Health Service Providers"
                  control={<Radio />}
                  label="Health Service Providers"
                />
                <FormControlLabel
                  value="Local Health Integration Network"
                  control={<Radio />}
                  label="Local Health Integration Network"
                />
                <FormControlLabel
                  value="Ontario Government"
                  control={<Radio />}
                  label="Ontario Government"
                />
                <FormControlLabel
                  value="Other User Groups"
                  control={<Radio />}
                  label="Other User Groups"
                />
              </RadioGroup>
            </FormControl>
          </Box>
      ...
      return (
        <div className="register__form">
          ...
          <Button
            ...
            onClick={handleSubmissionChange}
          >
            Add Submission
          </Button>
          <div className="register__tableContainer">
              ...
              actions={[
                {
                  ...
                  onClick: (event, rowData) => {
                    ...
                    console.log(rowData)
                    let editedPermission = userPermissions.splice(
...
const Register_container = (props) => {
  ...
  const {
    values: { organizations: userOrganizationsMap, programs: userProgramsMap },
    setFieldValue,
  } = props
  ...
  const columns = [
    ...
    { title: 'Authoritative Person Name', field: 'authoritativePersonName' },
    {
      ...
      field: 'authoritativePersonPhoneNumber',
    },
    {
      title: "Authoritative Person's Email",
      field: 'authoritativePersonEmail',
    },
  ]
  ...
  const handleNext = () => {
    if (activeStep === 0) {
      ...
      publicAxios
        .get(`${REST_PUBLIC_DATA}/organizations/${organizationGroup}`)
        .then(
          ({
            data: {
              data: { organizations },
            ...
            setUserOrganizations(
              Object.keys(userOrganizationsMap).map((_id) => ({
                ...userOrganizationsMap[_id],
                _id,
              }))
            )
          }
  ...
  const handleOrgChange = (event) => {
    ...
    publicAxios
      .get(`${REST_PUBLIC_DATA}/programs/${event.value}`)
      .then(
        ({
          data: {
            data: { programs },
          ...
          setUserPrograms(
            Object.keys(userProgramsMap).map((_id) => ({
              ...userProgramsMap[_id],
              _id,
            }))
          )
      ...
      .catch((error) => console.error(error))
    publicAxios
      .get(`${REST_PUBLIC_DATA}/organizations/information/${event.value}`)
      .then(
        ({
          data: {
            data: { organizations },
          ...
          let orgInformation = {
            ...
            authorizedName: organizations[0].contact.name,
            phone: organizations[0].contact.telephone,
            email: organizations[0].contact.email,
          }
  ...
  const handleOrgGroupChange = (event) =>
    setOrganizationGroup(event.target.value)
  ...
  const handleProgramChange = (selectedPrograms) => {
    ...
    publicAxios
      .post(`${REST_PUBLIC_DATA}/submissions`, { programList })
      .then(
        ({
          data: {
            data: { submissions },
