...
import {
  ROUTE_PUBLIC_REGISTER,
  ROUTE_PUBLIC_REGISTER_STEP2,
  ROUTE_PUBLIC_LOGIN,
  ROUTE_USER_PROFILE,
...
const RegisterFormContainer = (props) => {
  ...
  const [isOrganizationsDialogOpen, setIsOrganizationsDialogOpen] = useState(
    false
  )
  ...
  const {
    values: { organizations: userOrganizationsMap, roles },
    setFieldValue,
  } = props
  ...
  const handleDeleteRoleEntity = (role, entityType, entity) => {
    ...
    if (entityIndex < 0) {
      ...
      newEntities = [
        ...newEntities.slice(0, entityIndex),
        ...newEntities.slice(entityIndex + 1),
      ]
  ...
  const handleAddRoleEntity = (role, entityType, entity) => {
    ...
    if (entities.find(({ _id }) => _id === entity._id)) {
      ...
      updatedUserRoles[role][entityType] = [
        ...updatedUserRoles[role][entityType],
        entity,
      ]
  ...
  const handleDeleteUserOrganization = (userOrganization) => {
    ...
    const newUserOrganizations = [
      ...userOrganizations.slice(0, userOrganizationIndex),
      ...userOrganizations.slice(userOrganizationIndex + 1),
    ]
...
const EmailVerification = ({
  ...
  <Slide direction="left" in={visisble} mountOnEnter unmountOnExit>
    <Paper className="emailVerification">
      ...
      <ReturnButtons
        handleReturnToRegister={handleReturnToRegister}
