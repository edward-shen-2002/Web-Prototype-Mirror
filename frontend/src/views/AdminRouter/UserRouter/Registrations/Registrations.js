...
import { REST_PUBLIC_DATA, REST_ADMIN_REGISTRATIONS } from "constants/rest";
...
const Registrations = () => {
  ...
  const updateUser = (newUser, oldUser) => (
    adminUserRoleAxios.put(REST_ADMIN_REGISTRATIONS, { updatedUser: newUser })
      .then((response) => {
  ...
  const handleDeleteUser = (userToDelete) => {
    ...
    setUsers([ ...users.slice(0, userToDeleteIndex), ...users.slice(userToDeleteIndex + 1) ]);
  };
  ...
  const handleRowAdd = (newUser) => new Promise((resolve, reject) => {
    setTimeout(() => {
      adminUserRoleAxios.post(REST_ADMIN_REGISTRATIONS, { newUser })
        .then(({ data: { data: { user } } }) => {
          setUsers([ ...users, user ]);
          resolve();
        })
  ...
  const handleRowDelete = (oldUser) => new Promise((resolve, reject) => {
    setTimeout(() => {
      adminUserRoleAxios.delete(`${REST_ADMIN_REGISTRATIONS}/${oldUser._id}`)
        .then(() => {
          handleDeleteUser(oldUser);
          resolve();
        })
        .catch((error) => {
          console.error(error);
  ...
  const actions = [ 
    { icon: "check", tooltip: "Approve User", onClick: handleApproveUser },
  ...
  return (
    <div className="registrationsPage">
      <MaterialTable className="registrations__table" title="Registrations" columns={columns} actions={actions} data={users} editable={editable} options={options}/>
      <EntitiesDialog
