...
const MenuOptionItemIcon = ({ icon }) => (
  <ListItemIcon>
    {icon}
  </ListItemIcon>
);

const MenuOptionItemLabel = ({ label }) => (
  <ListItemText>
    {label}
  </ListItemText>
);
...
const MenuOptionsItems = ({ options }) => options.map(({ isDivider, icon, label, children, handleClick }, index) => (
  <Fragment key={index}>
    {
      isDivider
        ? <Divider/>
...
const Menu = ({ openedMenuName, label, options, handleClickMenu, handleHoverMenu }) => {
  ...
  return (
    <div>
      <MenuLabel isOpen={isOpen} label={label} handleClickMenuLabel={handleClickMenuLabel} handleHoverMenuLabel={handleHoverMenuLabel}/>
