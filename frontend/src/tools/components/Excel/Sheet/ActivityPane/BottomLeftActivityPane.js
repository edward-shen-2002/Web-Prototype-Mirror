...
const computeActiveCellStyle = (
  ...
  if (mergeData) {
    ...
    height =
      topOffsets[y2] + getNormalRowHeight(rowHeights[y2]) - topOffsets[y1]
    width =
      leftOffsets[x2] + getNormalColumnWidth(columnWidths[x2]) - leftOffsets[x1]
    top = topOffsets[y1]
    left = leftOffsets[x1]
  } else {
...
const BottomLeftActivityPane = () => {
  ...
  const isActiveCellInCorrectPane = (
    x,
    y,
  ...
  const isRelevantRowOffset = (rowOffset, freezeRowOffset) =>
    rowOffset > freezeRowOffset
