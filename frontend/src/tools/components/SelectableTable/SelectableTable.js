...
const CustomTableCells = ({ columns, item, props }) => columns.map(
  (column) => (
    <TableCell key={uniqid()} align="right" {...props}>{item[column.field]}</TableCell>
  )
)
...
const CustomListItems = ({ 
  ...
  handleSelect 
    ...
    const isSelected = useMemo(
      () => getKey && selectedKeys[getKey(item)],
      [ selectedKeys, getKey ]
    )

    const key = useMemo(
      () => getKey ? getKey(item) : uniqid(),
      [ getKey, item ]
    )

    return (
      <TableRow key={key} className={`list__item ${isSelected ? "list__item--selected" : ""}`} onClick={handleClick}>
