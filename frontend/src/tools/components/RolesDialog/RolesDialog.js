...
const formatRoleString = (role) =>
  role
    ...
    .map((string) =>
      string === 'LHIN'
        ? string
        : `${string.charAt(0)}${string.slice(1).toLocaleLowerCase()}`
    )
    .join(' ')

const RoleScope = ({
  currentroleScopeOption,
  roleScopesOptions,
  handleSelectRoleScope,
}) => (
  <div className="roleScope">
    <Chip
      className="roleScope__label"
      label="Scope"
      color="primary"
      icon={<SignalCellularAltIcon />}
    />
    <Select
      className="roleScope__select"
      value={currentroleScopeOption}
      options={roleScopesOptions}
      onChange={handleSelectRoleScope}
    />
  </div>
)
...
const HierarchyEntitiesButtons = ({
  ...
  /*handleClickLHINs,*/ handleClickSectors,
}) => (
  <ButtonGroup className="hierarchyButtons">
    <Button
      className="hierarchyButtons__button"
      color="secondary"
      variant="contained"
      onClick={handleClickOrganizations}
    >
      Organizations
    </Button>
...
const RoleListContent = ({
  role,
  roleContent: { scope, sectors, LHINs, organizations },
  handleSelectRoleScope,
  handleOpenEntityDialog,
}) => {
  const roleScopesOptions = ROLE_SCOPES.map((scope) => ({
    value: scope,
    label:
      scope === 'LHIN' || scope === 'N/A'
        ? scope
        : `${scope.charAt(0)}${scope.slice(1).toLocaleLowerCase()}`,
  }))

  const currentroleScopeOption = roleScopesOptions.find(
    ({ value }) => scope === value
  )

  const handleClickOrganizations = () =>
    handleOpenEntityDialog(role, 'organizations')
  ...
  return (
    <ListItem className="roleListContent">
      <RoleScope
        currentroleScopeOption={currentroleScopeOption}
        roleScopesOptions={roleScopesOptions}
        handleSelectRoleScope={handleSelectRoleScope}
      />
...
const RoleListItem = ({
  ...
  const handleSelectRoleScope = ({ value }) =>
    handleChangeRoleScope(role, value)

  return (
    <Fragment>
      <RolesListItemSummary
        isOpen={isOpen}
        role={formattedRoleString}
...
const RolesDialogContent = ({
  ...
  const handleOpenEntityDialog = (role, entityType) => {
    publicAxios
      .get(`${REST_PUBLIC_DATA}/${entityType}`)
      .then(({ data: { data } }) => {
  ...
  const handleAddEntity = (entity) =>
    handleAddRoleEntity(role, entityType, entity)

  const handleDeleteUserEntity = (entity) =>
    handleDeleteRoleEntity(role, entityType, entity)

  const userEntities = !role || !entityType ? [] : userRoles[role][entityType]

  const formattedEntityType = `${entityType
    .charAt(0)
    .toUpperCase()}${entityType.slice(1)}`
  const formattedRoleString = formatRoleString(role)
  ...
  const allSearchPlaceholder = `Search ${entityType.slice(
    0,
    entityTypeStringLength
  )}...`
