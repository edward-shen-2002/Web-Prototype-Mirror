...
import { closeTemplateTypeDialog } from '../../store/actions/DialogsStore'
...
const TemplateTypeDialog = ({ handleChange }) => {
  ...
  const { isTemplateTypeDialogOpen, templateTypes } = useSelector(
    ({
      DialogsStore: { isTemplateTypeDialogOpen },
      TemplateTypesStore: {
        response: { Values },
      },
    }) => ({
  ...
  const handleClose = useCallback(() => dispatch(closeTemplateTypeDialog()), [
    dispatch,
  ])
