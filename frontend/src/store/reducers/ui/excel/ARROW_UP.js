  ...
  if(shiftKey) { 
    if(stagnantSelectionAreasLength) {
      ...
      if(maxY > y && minArea.y2 !== maxY) {
        ...
        scrollTo(focusedStagnantSelectionArea.y2, y1 > y ? x1 : x2);
      ...
      const minArea = getWholeArea({ minX: x1, minY: y1, maxX: x2, maxY: y2, sheetCellData });

      if(y1 > 0) {
        newState.stagnantSelectionAreas = [ minArea ];
        newState.activeCellSelectionAreaIndex = 0;
        scrollTo(minArea.y1, x);
      } else {
        scrollTo(y, x);
      }
    }
    ...
    if(y > 0) newState.activeCellPosition = { x, y };
    newState.stagnantSelectionAreas = [];
    newState.activeCellSelectionAreaIndex = -1;
  }
};
