  ...
  if(x < startX || x > endX) {
    if(!isBounded) {
      if(shiftKey) {
        ...
        if(y >= startY) {
          ...
          if(rowData) {
            const { maxX, maxY } = getShiftTabFreeSpot({ y, startX, endX, rowData });
    ...
    if(stagnantSelectionAreas.length) {
      if(shiftKey) {
        ...
        if(currentRowData) {
          const { maxX: currentMaxX, maxY: currentMaxY } = getShiftTabFreeSpot({ endX: x, startX, y, rowData: currentRowData });
          ...
          if(currentMaxY !== y) {
            ...
            const { maxX, maxY } = getShiftTabFreeSpot({ endX, startX, y, rowData });
